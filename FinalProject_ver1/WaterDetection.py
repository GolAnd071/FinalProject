...
def detect_water(mode, in_filename, out_filename, product_type):
    """
    ...
    if mode == "Single":
        args = parser.parse_args(args=['-i', in_filename, '-o', out_filename, '-sm', '-p', product_type])
    else:
        args = parser.parse_args(args=['-i', in_filename, '-o', out_filename, '-p', product_type])
    ...
    if args.GetConfig:
        ...
        if (args.input is None) or (args.out is None):
            ...
            return waterdetect.DWWaterDetect.run_water_detect(input_folder=args.input,
